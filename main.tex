...
The goal of the \href{https://github.com/DamiFuma02/CurseOfDimensionality}{current study} \cite{repo_project} is to determine the best model architecture to learn some hidden (latent) features of the input images of the FashionMNIST dataset.
...
Setting a low dimensionality of the latent space, the effect of the non linearity will increase the accuracy and MSE scores much more than the linear solutions. \\
The space isn't regularized and has a lot of holes, so sampling an image from these holes will generate invalid data, tipically an overlap of multiple images. \\
...
The non linear dimensionality approaches are not reverisble, so starting from the latent space there's no way to reconstruct the original space.
For this reason this models are not evaluated into the current study, that focuses mainly on autoencoder models that require and encoder and a decoder.
...
In reality images are represented as a grid of pixels with a resolution $(Height \times Width)$ and a number of channels $C$ (RGB, ...) so the final image is a tensor with $(C,H,W) $ shapes.
...
The first layer identifies simple structures like edges and lines, the the deep layers extract more complex features, as a combination of the previous layers simple features.   \\
The convolution operations can be viewed as a specific case of a fully connected layer, in which the weight are all $0$ for the pixels that don't belong into the current sliding filter. \\
...
An important difference between CNN and ViT models are the number of parameters to train and floating point operations executed.
Since the CNN operates locally, the number of parameters is lower than ViT.
In many older CNN architectures like VGG-16, the final MLP layers actually contain over 90\% of the total parameters of the entire network because the flattened feature map at the end of the convolutions is quite large.
Modern designs (like ResNet) use Global Average Pooling to replace these dense layers, keeping the parameter count low throughout the entire model \cite{9687944}.
...
Since the FashionMNIST dataset contains images with simple pattern and textures, the initial architectures are capable enough to extract and learn the hidden features of the images, proven by low MSE and high SSIM scores for standard AE and low beta VAE. \\
